...
module Seasoning
  class << self
    ...
    def make_prototype_script(hostname, ruby_version, gem_version)
      hostname = hostname.gsub(/[\s_]/, "-")
      context = {:hostname => hostname, :ruby_version => ruby_version, :gem_version => gem_version}
    ...
    def redirect_command_output_to_brunch_log
      '> >(tee -a /var/log/brunch.log|logger -t brunch -s 2>/dev/console) 2>&1'
    end

    def redirect_script_output_to_brunch_log
      'exec ' + redirect_command_output_to_brunch_log
    end
